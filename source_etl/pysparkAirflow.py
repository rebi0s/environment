...
#CREATE TABLE bios.rebios.airflowTeste (location_id bigint NOT NULL, address_1 string, address_2 string, city string, state string, zip string, county string, location_source_value string, country_concept_id bigint, country_source_value string, latitude float, longitude float ) using iceberg;
...
if df_load.count() > 0:
    ...
    if df_location.count() > 0:
        ...
        count_max_location_df = spark.sql("SELECT greatest(max(location_id),0) + 1 AS max_location FROM bios.rebios.airflowTeste")
        ...
        df_location.show()   
        df_location.writeTo("bios.rebios.airflowTeste").append()
