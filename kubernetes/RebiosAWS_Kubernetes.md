# Deploying reBI0S on Kubernetes
This file describes the steps necessary steps to configure the reBI0S on Kubernetes using an EC2 instance.

## reBI0S Architecture
The rebI0S architecture is based on the state of the practice, open-source in Big Data architectures. 
...
sudo apt upgrade
```
### Next, install prerequisite packages that let apt use packages over HTTPS:
```
...
```
To avoid using microk8s as a prefix while running kubectl commands, you can add an alias of your preference if you don’t have an existing installation of kubectl.

### Here, we use "kc" as alias using the following command;
```
...
[The Superset Frontend](http://54.88.111.158:32378)<br>
user: <code> <b>admin</b></code><br>
pass: <code> <b>R4WCrL04cvA8</b></code><br>
...
[The Airflow Frontend](http://34.228.176.207:32163)<br>
user: <code> <b>airflow</b></code><br>
pass: <code> <b>airflow</b></code><br>
...
```
# fullnameOverride and nameOverride distinguish blank strings, null values,
...
singleuser:
  ...
  image:
    ...
    #tag: "4.0.0"
    name: rancavil/jupyterhub-docker
    tag: latest
    pullPolicy:
    pullSecrets: []
  startTimeout: 300
...
```
kc -n rebios-jupyter apply -f pv-jupyterhub.yaml
```
### Create the Volume Claim
file: pvc-jupyterhub.yaml
...
```
### Apply the Volume Claim

```
kc -n rebios-jupyter apply -f pvc-jupyterhub.yaml
```
### Install the JupyterHub 
```
helm upgrade --cleanup-on-fail \
  --install rebios jupyterhub/jupyterhub \
  --namespace rebios-jupyter \
  --version=4.0.0 \
  --values values.yaml
```

### Check JupyterHub Installation

```
kc -n rebios-jupyter get pods 

NAME                             READY   STATUS    RESTARTS   AGE
continuous-image-puller-jqps8    1/1     Running   0          12m
hub-548d8949c6-mk2gc             1/1     Running   0          12m
jupyter-admin                    1/1     Running   0          11m
proxy-67d4754895-6wscx           1/1     Running   0          12m
user-scheduler-8645f96f7-g6svd   1/1     Running   0          12m
user-scheduler-8645f96f7-qqf76   1/1     Running   0          12m

```

# Acess JupyterHub 
[The JupyterHub Frontend](http://54.88.111.158:30786)<br>
user: <code> <b>admin</b></code><br>
pass: <code> <b>change.it!</b></code><br>

![The JupyterHub Frontend](jupyter.jpg "The JupyterHub Frontend")
...
data:
  ...
  hue.ini: |
    ...
    [[[sparksql]]]
      name = Spark Sql
      interface=sqlalchemy
      options='{"url": "hive://spark-master-svc:10000/bios"}'
    ...
    sql_server_host=spark-master-svc
...
# Acess HUE 
[The HUE Frontend](http://54.88.111.158:32088)<br>
user: <code> <b>rebiosadmin</b></code><br>
pass: <code> <b>9t3GuWCf4M3D</b></code><br>
